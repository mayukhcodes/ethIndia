...
def transfer(sending_wallet,amount,asset_id,address):
    if address is None:
        return "Can you please specify Receiver address"
    try:
        transfer = sending_wallet.transfer(amount = transfer_amount,asset_id="eth",destination=address)
        transfer.wait()
        print(f"Transfer to {address} successful")
        print(f"Transaction link: {transfer.transaction_link}")
        print(f"Transaction hash: {transfer.transaction_hash}")
        ...
        transferResponse['link'] = transfer.transaction_link
        transferResponse['hash'] = transfer.transaction_hash
